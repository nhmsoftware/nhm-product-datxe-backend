...
final class AdminCreateRideBookingRequest extends FormRequest
{
    use HandleApi;
    ...
    public function rules(): array
    {
        return [
            ...
            'customer_id' => [
                'nullable',
                'string',
                Rule::requiredIf(fn (): bool => $this->input('customer_mode') === 'existing'),
                Rule::exists('users', 'id')->where(fn ($query) => $query
                    ->where('role', UserRole::Customer->value)
                    ->whereNull('deleted_at')),
            ],
            'customer_name' => ['nullable', 'string', 'max:100', Rule::requiredIf(fn (): bool => $this->input('customer_mode') === 'new')],
            'customer_phone' => ['nullable', 'string', 'regex:/^0[3-9]\d{8}$/', Rule::requiredIf(fn (): bool => $this->input('customer_mode') === 'new')],
    ...
    public function messages(): array
    {
        return [
            ...
            'customer_mode.required' => 'Vui lòng chọn loại khách hàng.',
            'customer_mode.in' => 'Loại khách hàng không hợp lệ.',
            'customer_id.required' => 'Vui lòng chọn khách hàng hiện có.',
            ...
            'airport_direction.in' => 'Chiều sân bay không hợp lệ.',
        ];
    }
